/**
 * 화면 코드의 직접 fetch·XHR 호출을 막는 프로젝트 전용 ESLint 규칙입니다.
 * 비즈니스 요청을 `src/lib/api.ts`의 apiFetch 경계로 모아 인증 토큰과 오류 처리를 일관되게 합니다.
 * 허용된 API·인증 어댑터 파일 이외의 네트워크 호출을 탐지합니다.
 */
...
function isAllowedFile(filename) {
  ...
  return (
    ...
    normalized.endsWith("/frontend/src/lib/api.ts") ||
    normalized.endsWith("/frontend/src/lib/auth.ts") ||
