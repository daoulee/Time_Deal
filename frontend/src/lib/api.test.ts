/**
 * apiFetch의 헤더·인증 토큰·요청 옵션 처리를 검증합니다.
 * pages가 사용하는 공통 네트워크 경계가 직접 요청으로 우회되지 않는지 확인합니다.
 * 실제 서버 대신 모의 fetch를 사용합니다.
 */
...
import { startThirdPartyGoogleAuth, syncAuthTokenFromUrl, uploadProductImage } from "@/lib/api";
import { clearAuthToken, getAuthToken } from "@/lib/auth";

const { storageFrom } = vi.hoisted(() => ({ storageFrom: vi.fn() }));
...
vi.mock("@/lib/supabase-auth", () => ({
  supabaseAuthClient: { storage: { from: storageFrom } }
}));
...
describe("uploadProductImage", () => {
  it("uses the server-selected bucket and Supabase signed upload contract", async () => {
    const uploadToSignedUrl = vi.fn(async () => ({ data: { path: "seller/product/image.png" }, error: null }));
    storageFrom.mockReturnValue({ uploadToSignedUrl });
    const file = new File(["image"], "image.png", { type: "image/png" });

    await expect(uploadProductImage("product-images", "seller/product/image.png", "signed-token", file)).resolves.toEqual({
      ok: true,
      status: 200,
      error: undefined
    });
  ...
  it("returns the storage error without reporting a successful upload", async () => {
    const uploadToSignedUrl = vi.fn(async () => ({ data: null, error: { message: "서명 토큰이 만료되었습니다." } }));
    storageFrom.mockReturnValue({ uploadToSignedUrl });
    const file = new File(["image"], "image.webp", { type: "image/webp" });

    await expect(uploadProductImage("product-images", "seller/product/image.webp", "expired-token", file)).resolves.toEqual({
      ok: false,
      status: 400,
      error: "서명 토큰이 만료되었습니다."
    });
  });
});
