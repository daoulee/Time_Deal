...
export async function getAdminItems(resource: "products" | "deals" | "reviews" | "community" | "community-reports" | "pickup-locations" | "pickup-slots" | "audit-logs" | "error-logs") { return requestData<{ items: RawRecord[] }>(`/admin/${resource}`); }
export async function reportClientError(message: string, stack?: string, path?: string) { return requestData<{ logged: boolean }>("/client-errors", { ...json("POST", { message, stack, path }), silent: true }); }
