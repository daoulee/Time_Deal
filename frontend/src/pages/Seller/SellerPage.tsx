...
function ProductForm({ onSaved }: { onSaved?: () => void }) {
  const emptyForm = { name: "", description: "", category: "", regularPrice: 0, inventory: 0, dealPrice: 0, target: 1, startsAt: "", endsAt: "", autoDiscountEnabled: true, autoDiscountStartHours: 3, autoDiscountMaxPercent: 15 };
  ...
  return <section className="dashboard-panel"><div className="panel-title-row"><div><p>NEW PRODUCT</p><h2>상품·타임딜 등록</h2></div><StatusBadge type="live">즉시 판매 시작</StatusBadge></div><Feedback error={error} notice={notice} /><form className="operation-form" onSubmit={submit}><label>상품명<input required minLength={2} value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} /></label><label>카테고리<input required value={form.category} onChange={(event) => setForm({ ...form, category: event.target.value })} /></label><label>정상가<input required min={0} type="number" value={form.regularPrice} onChange={(event) => setForm({ ...form, regularPrice: Number(event.target.value) })} /></label><label>초기 재고<input required min={0} type="number" value={form.inventory} onChange={(event) => setForm({ ...form, inventory: Number(event.target.value) })} /></label><label className="wide">설명<textarea required minLength={2} value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} /></label><label className="wide">상품 사진{imagePreview && <div className="image-preview-row"><img src={imagePreview} alt="상품 미리보기" className="product-image-preview" /><button type="button" className="image-remove-button" onClick={() => pickImage(null)} aria-label="상품 사진 삭제"><X size={14} /></button></div>}<label className="file-action"><ImageUp size={15} /> {imageFile ? "사진 변경" : "사진 선택"}<input type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => pickImage(event.target.files?.[0] ?? null)} /></label></label><label>딜 가격<input required min={0} max={form.regularPrice} type="number" value={form.dealPrice} onChange={(event) => setForm({ ...form, dealPrice: Number(event.target.value) })} /></label><label>목표 수량<input required min={1} type="number" value={form.target} onChange={(event) => setForm({ ...form, target: Number(event.target.value) })} /></label><label>딜 시작<input required type="datetime-local" value={form.startsAt} onChange={(event) => setForm({ ...form, startsAt: event.target.value })} /></label><label>딜 종료<input required type="datetime-local" value={form.endsAt} onChange={(event) => setForm({ ...form, endsAt: event.target.value })} /></label><p className="wide" style={{ margin: "8px 0 0", fontSize: 13, fontWeight: 700, color: "#1a202c" }}>마감 임박 자동 할인율</p><label className="wide check-label"><input type="checkbox" checked={form.autoDiscountEnabled} onChange={(event) => setForm({ ...form, autoDiscountEnabled: event.target.checked })} /> 마감이 가까워질수록 자동으로 추가 할인 (설정 안 하면 마감 3시간 전부터 최대 15%p 추가 할인)</label>{form.autoDiscountEnabled && <><label>할인 시작 시점 (마감 몇 시간 전부터)<input required min={0} max={168} type="number" value={form.autoDiscountStartHours} onChange={(event) => setForm({ ...form, autoDiscountStartHours: Number(event.target.value) })} /></label><label>최대 추가 할인율 (%)<input required min={0} max={90} type="number" value={form.autoDiscountMaxPercent} onChange={(event) => setForm({ ...form, autoDiscountMaxPercent: Number(event.target.value) })} /></label><p className="wide muted-copy" style={{ margin: 0, fontSize: 11 }}>딜 가격({form.dealPrice ? formatPrice(form.dealPrice) : "0원"})에서 시작해서, 마감 {form.autoDiscountStartHours}시간 전부터 서서히 최대 {form.autoDiscountMaxPercent}%p까지 추가로 할인됩니다. 사장님이 따로 손댈 건 없고, 화면에 보이는 가격만 자동으로 내려갑니다.</p></>}<button className="primary-button" disabled={busy}>{busy && <LoaderCircle className="spin-icon" size={16} />}상품·딜 등록</button></form></section>;
}

function ProductsPanel() {
  const [items, setItems] = useState<RawRecord[]>([]); const [deals, setDeals] = useState<RawRecord[]>([]); const [loading, setLoading] = useState(true); const [busy, setBusy] = useState<string | null>(null); const [error, setError] = useState<string | null>(null); const [notice, setNotice] = useState<string | null>(null); const [history, setHistory] = useState<Record<string, RawRecord[]>>({});
  const load = useCallback(async () => {
    setLoading(true);
    const [productsResult, dealsResult] = await Promise.all([getSellerProducts(), getSellerDeals()]);
    setLoading(false);
    if (productsResult.ok) setItems(productsResult.data?.products ?? []); else setError(productsResult.error?.message ?? "상품을 불러오지 못했습니다.");
    if (dealsResult.ok) setDeals(dealsResult.data?.deals ?? []);
  }, []);
  useEffect(() => { void load(); }, [load]);
  const activeDealByProduct = useMemo(() => {
    const map = new Map<string, RawRecord>();
    for (const deal of deals) if (["draft", "active"].includes(str(deal, "status"))) map.set(str(deal, "product_id"), deal);
    return map;
  }, [deals]);
...
function SellerRestockRequestsPanel() {
  ...
  const reply = async (id: string) => {
    const form = replies[id]; const message = form?.message?.trim();
    if (!message) { setError("답변 내용을 입력하세요."); return; }
    setBusy(id); setError(null);
    const result = await replySellerRestockRequest(id, { sellerReply: message, expectedRestockDate: form?.date || undefined });
    setBusy(null);
  ...
  return <section className="dashboard-panel"><div className="panel-title-row"><div><p>RESTOCK REQUESTS</p><h2>재입고 요청</h2></div><button className="secondary-button" onClick={() => void load()}><RefreshCw size={15} /> 새로고침</button></div><Feedback error={error} notice={notice} />
    ...
    {items.length === 0 ? <Empty title="재입고 요청이 없습니다." text="고객이 주문 내역에서 재입고를 요청하면 여기에 표시됩니다." /> : <div className="operation-list">{items.map((item) => {
      ...
      return <article className="operation-card" key={str(item, "id")}>
        <header><div><strong>{str(products, "name") || "상품"}</strong><small>{str(profiles, "name") || "고객"} · {date(str(item, "created_at"))}</small></div><StatusBadge type={answered ? "live" : "ready"}>{answered ? "답변 완료" : "답변 대기"}</StatusBadge></header>
        {str(item, "message") && <p className="muted-copy">"{str(item, "message")}"</p>}
        {answered ? <div className="compact-row"><span><b>내 답변</b><small>{str(item, "seller_reply")}</small></span>{str(item, "expected_restock_date") && <strong>예상 입고일 {str(item, "expected_restock_date")}</strong>}</div> : <div className="operation-form compact">
          <label>예상 입고일 (선택)<input type="date" value={replies[str(item, "id")]?.date ?? ""} onChange={(event) => setReplies({ ...replies, [str(item, "id")]: { date: event.target.value, message: replies[str(item, "id")]?.message ?? "" } })} /></label>
          <label>답변<textarea rows={2} value={replies[str(item, "id")]?.message ?? ""} onChange={(event) => setReplies({ ...replies, [str(item, "id")]: { date: replies[str(item, "id")]?.date ?? "", message: event.target.value } })} placeholder="곧 다시 들어올 예정이에요!" /></label>
          <button className="primary-button" disabled={busy === str(item, "id")} onClick={() => void reply(str(item, "id"))}>{busy === str(item, "id") ? "저장 중..." : "답변 보내기"}</button>
        </div>}
      </article>;
    })}</div>}
  </section>;
