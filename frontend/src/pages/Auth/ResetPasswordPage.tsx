...
export default function ResetPasswordPage() {
  ...
  const submit = async (event: FormEvent) => { event.preventDefault(); if (password !== confirm) return toast.error("비밀번호 확인이 일치하지 않습니다."); if (!supabaseAuthClient) return toast.error("인증 환경변수를 설정하세요."); setSubmitting(true); const { error } = await supabaseAuthClient.auth.updateUser({ password }); if (error) toast.error(translateAuthErrorMessage(error.message, "비밀번호를 변경하지 못했습니다.")); else { await supabaseAuthClient.auth.signOut({ scope: "global" }); toast.success("비밀번호를 변경했습니다. 새 비밀번호로 로그인해 주세요."); navigate("/auth", { replace: true }); } setSubmitting(false); };
