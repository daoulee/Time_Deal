  ...
  return (
    <div className="site-frame">
      ...
      <header className="site-header">
        ...
        {open && <nav className="mobile-nav" aria-label="모바일 메뉴">{links.map(([to, label]) => <NavLink key={to} to={to} onClick={() => setOpen(false)}>{label}</NavLink>)}<Link to="/seller" onClick={() => setOpen(false)}>판매자 센터</Link></nav>}
      </header>
      <main id="main-content">{children}</main>
      <footer className="site-footer"><div><Link to="/" className="footer-brand">타임딜</Link><p>이웃과 함께 만드는 합리적인 시간 한정 공동구매</p></div><div className="footer-links"><Link to="/seller">판매자 센터</Link><Link to="/admin">관리자 콘솔</Link><Link to="/inquiry">고객센터</Link></div><p className="footer-note">온라인 PG와 네이버 로그인은 외부 계약·별도 연동 후 활성화하며, 현재 주문은 현장 결제 또는 결제 없는 예약으로 운영합니다.</p></footer>
    </div>
  );
}
