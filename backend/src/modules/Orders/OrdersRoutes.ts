...
const createSchema = z.object({ pickupLocationId: z.string().uuid(), pickupSlotId: z.string().uuid(), paymentMethod: z.enum(["on_site", "reservation_only"]), idempotencyKey: z.string().min(8).max(120), items: z.array(itemSchema).min(1).max(50) }).strict().refine((value) => new Set(value.items.map((item) => item.productId)).size === value.items.length, "같은 상품을 중복해서 담을 수 없습니다.");
...
ordersRouter.post("/orders", async (context) => {
  ...
  return context.json(apiSuccess({ order: current ? serializeOrder(current) : null, replayed: Boolean(result.replayed), payment: { method: parsed.data.paymentMethod, status: parsed.data.paymentMethod === "on_site" ? "pay_on_pickup" : "not_applicable", provider: null }, source: "supabase" }), result.replayed ? 200 : 201);
});
ordersRouter.post("/orders/:id/cancel", async (context) => {
  ...
  const { data, error } = await getAdminSupabase().rpc("cancel_order_atomic", { p_order_id: context.req.param("id"), p_actor_id: context.var.currentUser.id });
  return error ? context.json(apiFailure("ORDER_CANCEL_FAILED", error.message), 409) : context.json(apiSuccess({ ...(data as object), payment: { status: row.payment_status, refund: "not_applicable" }, source: "supabase" }));
});
...
ordersRouter.get("/seller/orders", requireRole("seller"), async (context) => {
  ...
  const [itemsResult, ordersResult] = await Promise.all([supabase.from("order_items").select("id,fulfillment_group_id,product_id,product_name,quantity,unit_price,line_total").in("fulfillment_group_id", groupIds), supabase.from("orders").select("id,order_number,pickup_location_id,pickup_slot_id,payment_method,created_at").in("id", [...new Set((groups ?? []).map((group) => group.order_id))])]);
  if (itemsResult.error || ordersResult.error) return context.json(apiFailure("QUERY_FAILED", "판매자 주문 상세를 조회하지 못했습니다."), 502);
  const fulfillments = (groups ?? []).map((group) => ({ id: group.id, orderId: group.order_id, orderNumber: ordersResult.data?.find((order) => order.id === group.order_id)?.order_number, pickupLocationId: ordersResult.data?.find((order) => order.id === group.order_id)?.pickup_location_id, pickupSlotId: ordersResult.data?.find((order) => order.id === group.order_id)?.pickup_slot_id, paymentMethod: ordersResult.data?.find((order) => order.id === group.order_id)?.payment_method, status: group.status, pickupStatus: group.pickup_status, subtotal: group.subtotal, createdAt: group.created_at, items: itemsResult.data?.filter((item) => item.fulfillment_group_id === group.id) ?? [] }));
