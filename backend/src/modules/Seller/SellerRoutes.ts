...
sellerRouter.post("/seller-products-with-deal", sellerOnly, async (context) => {
  ...
  const { data: deal, error: dealError } = await supabase.from("deals").insert({ product_id: product.id, deal_price: dealPrice, target, starts_at: startsAt, ends_at: endsAt, status: "active" }).select().single();
...
sellerRouter.post("/seller-deals", sellerOnly, async (context) => {
  ...
  const { data, error } = await getAdminSupabase().from("deals").insert({ product_id: parsed.data.productId, deal_price: parsed.data.dealPrice, target: parsed.data.target, starts_at: parsed.data.startsAt, ends_at: parsed.data.endsAt, status: "active" }).select().single();
