...
export const optionalAuth:MiddlewareHandler=async(c,next)=>{c.set("currentUser",null); const h=c.req.header("Authorization"); if(!h?.startsWith("Bearer ")) return next(); try{const sb=getAdminSupabase(); const {data,error}=await sb.auth.getUser(h.slice(7)); if(error||!data.user) return c.json(apiFailure("INVALID_SESSION","세션이 만료되었거나 유효하지 않습니다."),401); const {data:p}=await sb.from("profiles").select("name,role").eq("id",data.user.id).maybeSingle(); c.set("currentUser",{id:data.user.id,email:data.user.email??"",name:p?.name??String(data.user.user_metadata?.name??"사용자"),emailVerified:Boolean(data.user.email_confirmed_at),role:(p?.role??"user") as AppRole}); return next()}catch{return c.json(apiFailure("SUPABASE_UNCONFIGURED","Supabase 환경변수를 먼저 설정하세요."),503)}};
