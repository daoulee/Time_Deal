...
- **내 정보(MyPage) 화면 토스/당근/iOS 스타일 그룹형 카드 전면 모더니제이션**:
  - 프로필 카드 + 에코 절약 배너 + 3분할 스탯 + 컬러풀 아이콘 배지 그룹형 카드 목록 전면 개편
- **Apple Liquid Glass + Toss Micro-Interactions 플로팅 캡슐 내비게이션 바 전면 구현**:
  - 다이나믹 아일랜드 스타일의 플로팅 캡슐(`BackdropFilter` 블러 + 흑요석 유리 질감), 탭 시 화이트 스퀴클 인디케이터 부드러운 스프링 전환 및 햅틱 피드백 연동
